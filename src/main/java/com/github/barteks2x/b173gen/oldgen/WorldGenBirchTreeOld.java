...
public class WorldGenBirchTreeOld implements WorldGenerator173 {
    ...
    public boolean generate(ISimpleWorld world, Random random, int i, int j, int k) {
            ...
            for(int i2 = i - byte0; i2 <= i + byte0 && flag; i2++) {
                for(int l2 = k - byte0; l2 <= k + byte0 && flag; l2++) {
                    if(i1 >= 0 && i1 < WorldConfig.heightLimit) {
                        ...
                        if(j3 != Material.AIR && j3 != Material.BIRCH_LEAVES) {
                            flag = false;
                        }
                    } else {
                        flag = false;
            ...
            for(int k3 = i - i3; k3 <= i + i3; k3++) {
                ...
                for(int i4 = k - i3; i4 <= k + i3; i4++) {
                    ...
                    if((Math.abs(l3) != i3 || Math.abs(j4) != i3 || random.nextInt(2) != 0 && j2 != 0)) {
                        world.setType(k3, k1, i4, Material.BIRCH_LEAVES, new Leaves(TreeSpecies.BIRCH));
                    }
                }
        ...
        for(int l1 = 0; l1 < l; l1++) {
            ...
            if(k2 == Material.AIR || k2 == Material.BIRCH_LEAVES) {
                world.setType(i, j + l1, k, Material.BIRCH_LOG, new Tree(TreeSpecies.BIRCH));
            }
        }
