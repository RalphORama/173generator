...
public class WorldGenCavesOld extends WorldGenBaseOld {
    ...
    protected void generateDefaultBranch(int generatedChunkX, int generatedChunkZ, ChunkGenerator.ChunkData data, double structureOriginBlockX, double structureOriginBlockY, double structureOriginBlockZ) {
        generateBranch(
    ...
    protected void generateBranch(int generatedChunkX, int generatedChunkZ,
        ...
        for (; currentCaveSystemRadius < maxCaveSystemRadius; currentCaveSystemRadius++) {
            ...
            double caveRadiusHorizontal = MIN_HORIZONTAL_SIZE + (double) (sin((float) currentCaveSystemRadius * 3.141593F / (float) maxCaveSystemRadius) * maxHorizontalSize * 1.0f);
                    ...
                    for (int localY = endY; localY >= startY; localY--) {
                        ...
                        if (yDistanceScaled > -0.7D &&
                            ...
                            if (previousBlock == STONE
                                    ...
                                    || previousBlock == GRASS) {
                                if (localY < 10) {
                                    data.setBlock(localX, localY + 1, localZ, LEGACY_STATIONARY_LAVA);
                                } else {
            ...
            for (int zPos = startZ; zPos < endZ; zPos++) {
                for (int yPos = endY + 1; yPos >= startY - 1; yPos--) {
                    ...
                    if (data.getType(xPos, yPos, zPos) == WATER
                            || data.getType(xPos, yPos, zPos) == LEGACY_STATIONARY_WATER) {
                        return true;
                    }
